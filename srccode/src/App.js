...
function App() {
  ...
  const handleSubmit=(e)=>{
    e.preventDefault()
    if(tmp!=="" && tmp.toLowerCase().includes("@gmail.com") && tmp.toLowerCase().indexOf("@")===tmp.toLowerCase().lastIndexOf("@") && tmp.toLowerCase().indexOf("gmail")===tmp.toLowerCase().lastIndexOf("gmail") && tmp.toLowerCase().indexOf(".")===tmp.toLowerCase().lastIndexOf(".") && tmp.toLowerCase().indexOf("com")===tmp.toLowerCase().lastIndexOf("com") ){
      setEmail(tmp)
      console.log(email)
      setTmp("")
      setError(false)
  ...
  return (
    <div className='box'>
      ...
      <img src={(width<500)?require("./assets/hero-mobile.jpg"):require("./assets/hero-desktop.jpg")} alt="img" className='img' />
        <div className="text">
        
        <h1 className='head'>WE'RE <em>COMING SOON</em></h1>
        <p className='about'>Hello fellow shoppers! We're currently building our new fashion store. Add your email below to stay up to date with announcements and our launch deals.</p>

        <form onSubmit={handleSubmit} className="form">
          <div className="submit">
          <div className={(!error)?"input":"input error-input"}>
            <input type="text" name="email" className={(!error)?"email":"email error-mail"} placeholder='Email Address' value={tmp} onChange={(e)=>setTmp(e.target.value)}/>
            
          </div>
          <button type="submit" onClick={()=>setActive(true)} className="btn">
          ...
          {(error)?<p className='error'>Please Provide a valid Email.</p>:<p className='success'>{(active)?"Successful Submission":""}</p>}
